...
function SWEP:PrimaryAttack()
    ...
    -- Call 'ThrowBarrel' on self with this model
    self:ThrowBarrel("weapon_zs_stone")
end

-- A custom function we added. When you call this the player will fire a barrel!
function SWEP:ThrowBarrel(model_file)
    local owner = self:GetOwner()

    -- Make sure the weapon is being held before trying to throw a barrel
    ...
    local pos = aimvec * 50 -- This creates a new vector object
    ...
    -- Now we apply the force - so the barrel actually throws instead 
